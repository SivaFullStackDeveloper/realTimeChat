...
router.post('/upload', upload.single('file'), function (req, res, next) {
    if (req.file) {
        //
        var url = 'https://chat.cartravels.com/uploads/' + req.file.filename
      
        return res.send({ success: true, data: url, message: "Upload successfull" })
    }
    else {
...
router.post('/createcitygroup', (req, res, next) => {
    groupController.createCityGroup(req.body).then((data) => {
        groupController.addUser(data._id, data.room, req.body.user.phone,req.body.user.uid).catch(console.log);
...
router.post('/creategroup', (req, res, next) => {
    groupController.createGroup(req.body).then((data) => {        
       groupController.addUser(data._id, data.room, req.body.admin.phone).catch(console.log);
        // req.body.contacts.forEach((e) => {
        //     groupController.addUser(data._id, data.room, e.phone,e.uid).catch(console.log);
        // });
        res.send({ error:false,data })
    });
});
...
router.get('/chatHome', (req, res, next) => {
    if (req.query.user == undefined || req.query.user == "") {
        ...
        req.query.image = "https://play-lh.googleusercontent.com/ha3sgGB97PZgAPez1VAV8qQYuNDslIQUPfSAeZXBuSrZMjGw140DRGizreqMO45tEwU=s180-rw";
        groupController.createCityGroup(req.query).then((data) => {
           groupController.addUser(data._id, data.room, req.query.user,req.query.uid).catch(console.log);
        });
        groupController.getUserRooms(req.query.user).then((groupData) => {
            ...
            chatController.getLatestChatsByUser(req.query.user, rooms).then((data) => {
                var result = [];
                data.forEach((ele) => {
                    ...
                    var ind = result.findIndex((rData) => rData.message.room == ele.room);
                    if (ind == -1) {
                        result.push(t);
                    }
                    else {
